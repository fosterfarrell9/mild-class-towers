...
The complete enumeration behind the paper: all imaginary quadratic
fields K with |D_K| < 2^30 whose p-class rank is three for an odd
prime p.

```text
p3-discriminants.txt    12 749 fields   (p = 3)
p5-discriminants.txt       204 fields   (p = 5)
p7-discriminants.txt         3 fields   (p = 7)
```

One discriminant per line, by increasing |D_K|.  The lists are
derived from the directory names under `certificates/` (via the
radicand at p = 5) and are exactly the fields whose certificates the
repository stores; no other odd prime admits a field of p-class rank
three in this range.

The enumeration is read off from the unconditional class-group
tables of Mosunov--Jacobson; `mosunov-jacobson/README.md` records
their provenance, the decoding convention, the parser, and the
committed validation artifacts.  Independently of the tables, the C
program contains a scanner that recomputes class groups directly
with PARI's `quadclassunit`; the two agree on every committed
comparison range (`p5-r3/scan-1-25000000.gp` against
`mosunov-jacobson/p5-rank-ge3-1-25000000.tsv`).  The remainder of
this document describes that scanner.

## Candidate discovery with the scanner

Candidate discovery is an inexpensive filter, separate from the
audited secondary-norm and mildness computation.  A rank-three
candidate is not thereby claimed to be mild.
...
./build/massey --scan-candidates \
  ...
  --progress-seconds 10 \
  --output census/p5-r3-1-100000.gp
```
...
```sh
gp -q
? r=read("census/p5-r3-1-100000.gp");
? r[10][2]
```
...
```sh
./build/massey --candidate-inputs census/p5-r3-1-100000.gp
```
