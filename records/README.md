...
What the searches produced, and what follows from the verified
matrices — separated by prime.  `p3/` holds the source tensors, the
verification records, the strong-freeness verdicts with their
second-engine crosschecks, and the cone-criterion reports; `p5/` the
per-field result records with run logs and the transverse rank-one
certificates.  The three p = 7 fields keep their search records
(matrices, transversality protocols) beside their certificates in
`certificates/p7/`.  The arithmetic certificates of all three primes
live under `certificates/` at the repository root.

The remainder of this document describes the compact p = 5 result
records and the batch runner that produced them.  They are compact,
machine-readable computations of imaginary quadratic examples. The detailed release in
`certificates/p5/K-2800905-p5/` remains the reference certificate: compact records
...
```sh
mkdir -p records/p5/D-11203620
./build/massey --example-result \
  records/p5/D-11203620/result.gp 5 's^2+2800905'
```
...
```sh
./build/massey --example-result \
  records/p5/D-18397407/result.gp 5 's^2-s+4599352'
```
...
python3 tools/run_mildness_batch.py \
  --batch records/p5/batch-block0-01
```
...
```sh
python3 tools/run_mildness_batch.py \
  --batch records/p5/batch-block0-01 --resume
```
